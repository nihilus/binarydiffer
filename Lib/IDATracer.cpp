...
int SharedSocketDataReceiver(SOCKET data_socket,char type,DWORD length,PBYTE data)
{
	msg("Got Data type=%d length=%d\n",type,length);
	return 1;
}
...
void DumpToSharedSocket(int type,int level,ea_t current_address,char *disasm_line)
{
	...
	if(type==TRACE_TYPE_END)
	{
		SendTLVData(data_socket,TRACE_TYPE_END,(PBYTE)"",0);
		return;
	}
	if(type==TRACE_TYPE_START)
	{
		char Filename[MAX_PATH+1];
		_snprintf(Filename,sizeof(Filename),"Trace-%.8x.txt",current_address);
		data_socket=ConnectToServer("127.0.0.1",1111);
		SetSharedSocketDataReceiver(SharedSocketDataReceiver);
		PutSocketToWSAAsyncSelect(data_socket,SharedSocketDataReceiverWndProc,WM_SHARED_SOCKET_EVENT);
		return;
	}
	if(type==TRACE_TYPE_DISASM_LINE && data_socket!=INVALID_SOCKET)
	{
		//level,current_address,disasm_line
		int trace_message_length=sizeof(TraceMessage)+strlen(disasm_line)+1;
		TraceMessage *p_trace_message=(TraceMessage *)malloc(trace_message_length);
		p_trace_message->level=level;
		p_trace_message->current_address=current_address;
		p_trace_message->disasm_line_len=strlen(disasm_line)+1;
		memcpy(p_trace_message->disasm_line,disasm_line,strlen(disasm_line)+1);
		SendTLVData(data_socket,TRACE_TYPE_DISASM_LINE,(PBYTE)p_trace_message,trace_message_length);
	}
}
...
void DumpMapDataToSharedSocket(char type,PBYTE data,int length)
{
	...
	msg("Sending [type=%d data=%x(%s) length=%d]\n",type,data,data,length);
	if(!SendTLVData(data_socket,type,data,length))
	{
...
{
	static IMapDrawer *IMapDrawerInstance=NULL;
	static DWORD current_address=0;
	static char *current_function_name=NULL;

	HRESULT hr;
	if(IMapDrawerInstance==NULL)
	{
		hr=CoInitialize(0);
		msg("CoInitialize\n");
		if(SUCCEEDED(hr))
		{
			msg("CoCreateInstance\n");
			hr=CoCreateInstance(CLSID_MapDrawer,NULL,CLSCTX_LOCAL_SERVER,IID_IMapDrawer,(void**)&IMapDrawerInstance); 
		}	
	}

	if(type==TYPE_MAP_DATA && length>=sizeof(DWORD)*2)
	{
		DWORD src;
		DWORD dst;
		memcpy(&dst,data,sizeof(DWORD));
		memcpy(&src,(char *)data+sizeof(DWORD),sizeof(DWORD));
		hr=IMapDrawerInstance->SetMapData(src,dst);
	}else if(type==TYPE_CURRENT_ADDRESS)
	{
		current_address=*((DWORD *)data);
	}else if(type==TYPE_FUNCTION_NAME)
	{
		current_function_name=strdup((char *)data);
	}else if(type==TYPE_DISASSEMBLY)
	{
		char node_name[1024];
		_snprintf(node_name,sizeof(node_name),"%x",current_address);
		BSTR bstr_node_name=SysAllocString((const OLECHAR *)node_name);
		BSTR bstr_current_function_name=SysAllocString((const OLECHAR *)current_function_name);
		BSTR bstr_disassembly_list=SysAllocString((const OLECHAR *)data);
		hr=IMapDrawerInstance->SetNodeData(current_address,bstr_node_name,bstr_current_function_name,bstr_disassembly_list);
		if(current_function_name)
			free(current_function_name);
	}else if(type==TYPE_END)
	{
		HWND message_window=CreateMessageWindow(MessageWndProc);	
		IMapDrawerInstance->SetCallbackWindowMessage((DWORD)message_window,WM_DRAW_DATA);

		hr=IMapDrawerInstance->Display();
		//IMapDrawerInstance->Release();
	}
	if(!SUCCEEDED(hr))
	{
		IMapDrawerInstance=NULL;
		CoUninitialize();
	}
}
...
multimap <ea_t,ea_t> *TraceVariable(ea_t start_address,int operand_pos,bool b_trace_up,bool b_trace_passive_usage)
{
	...
	msg("%x operand=%d\n",start_address,operand_pos);
	DumpCurrentOptInfo(start_address,operand_pos);
	...
	memcpy((void *)&analysis_element.operand,(void *)&cmd.Operands[operand_pos],sizeof(op_t));
